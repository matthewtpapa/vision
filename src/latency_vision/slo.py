    ...
    *,
    ...
    if recall < gates.offline_recall:
        raise AssertionError(
            f"offline recall {recall:.4f} < gate {gates.offline_recall:.4f}"
        )
    if lookup_p95 > gates.offline_p95_ms:
        raise AssertionError(
            f"offline p95 {lookup_p95:.4f}ms > gate {gates.offline_p95_ms:.4f}ms"
        )
    p_at_1 = float(p_at_1)
    ...
    if e2e_p95 > gates.e2e_p95_ms:
        raise AssertionError(
            f"e2e p95 {e2e_p95:.4f}ms > gate {gates.e2e_p95_ms:.4f}ms"
        )
